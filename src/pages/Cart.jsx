...
import { checkout, deleteProduct, minusCart, plusCart } from "../features/Products/cartSlice"
import { minusProduct, plusProduct, refreshProduct } from "../features/Products/ProductsSlice"
...
const Cart = () => {
  ...
  const hdlDelete = (prod) => {
    dispatch(deleteProduct(prod))
    dispatch(refreshProduct(prod))
  }
    ...
    <>
      <motion.main
          ...
          {cartItems && cartItems.map(product => (
              <motion.div exit={{x: -300}} transition={{type: 'tween'}} layout  key={product.id} className="w-full h-fit gap-5 p-5 font-quicksand dark:bg-transparent flex hpk:flex-row flex-col justify-between items-start hpk:items-center border-b-2 border-b-noHover border-opacity-20">
                ...
                <div className="flex flex-row justify-between hpk:justify-end items-center gap-4 w-full hpk:w-auto hpk:gap-8 self-end">
                  <svg onClick={() => hdlDelete(product)} className="fill-noHover dark:fill-HDark self-end" width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
