...
import { changeAddress, changePay, selectAll } from "../features/Products/cartSlice"
        ...
        <aside className={`hpk:w-[380px] w-full fixed md:sticky h-screen bg-white md:bg-transparent md:h-full  ${payIsOpen? 'top-24 hpsk:top-20' : 'top-[calc(100vh-60px)]'}  right-0 hpk:right-3 md:top-24  rounded-xl overflow-hidden shadow-xl transition-all duration-300`} id="aside">
          ...
          <div className="px-4 py-4 bg-white dark:bg-darkBg flex flex-col gap-5">
            ...
            <div className="w-full py-3 px-6 bg-noHover dark:bg-darkNav bg-opacity-10 rounded-lg">
              <select onChange={(e) => dispatch(changePay(e.target.value))} placeholder="Payment Method" name="" id="" className="w-full outline-none bg-transparent p-0 font-quicksand text-xl font-semibold text-HLig dark:text-HDark ">
                <option value="NoPayment" className="text-noHover">Payment Method</option>
                <h1>hehe</h1>
                <option value="Paypal" className="text-HLig">Paypal
                
                </option>
                <option value="Go-pay" className="text-HLig">Go-pay</option>
                <option value="Mandiri" className="text-HLig">Mandiri</option>
                <option value="Visa" className="text-HLig">Visa</option>
              </select>
            </div>
            <div className=" w-full px-6 py-3 bg-noHover dark:bg-darkNav bg-opacity-10 rounded-lg">
              ...
              <textarea onChange={(e) => dispatch(changeAddress(e.target.value))} name="" id="Shipping"  className="bg-transparent border-none outline-none w-full font-quicksand font-semibold text-base h-40 rounded-lg mt-3 text-TexDark" placeholder="Add Address"></textarea>
            </div>

            {jumlahCek > 0 ? (
              <button onClick={() => {
                if(totalCartQuantity > 0) {
                  if(pay == 'NoPayment' || address == '') {
                    toast.error('Harap isi bidang')
                  } else {
                    dispatch(changeIsConfPay(totalCartAmount))
                  }
                } 
