  ...
  const { filter } = useSelector(state => state.route)
	...
	return (
        ...
        animate={{opacity: 1, transition: {duration: .7, ease: 'easeOut', type: 'tween'}}}
				exit={{opacity: 0}}
        className="min-h-[300vh] flex flex-row pt-[80px]" id="main">
				<motion.aside
					initial={{opacity: 0}}
					animate={{opacity: 1, transition: {delay: .2, duration: .5, ease: 'easeIn'}}}
					className={`fixed shadow-xl ${filter? 'left-0' : '-left-80' } transition-all duration-300 md:shadow-none md:left-0 md:sticky w-80 h-[800px] top-[80px] bg-white dark:bg-darkBg px-16 pt-24 z-30`} id="filter-card">
					<div className="font-quicksand text-lg font-medium text-HLig dark:text-HDark flex flex-col items-start gap-2">
						<div className="flex items-center gap-2">
							<input type="checkbox" name="" id="mx100" className="accent-navigator w-4 h-4 rounded-full" />
							<label htmlFor="mx100">Max. $ 20.00</label>
						</div>
						<div className="flex items-center gap-2">
							<input type="checkbox" name="" id="mn100" className="accent-navigator w-4 h-4 rounded-full" />
							<label htmlFor="mn100">Min. $ 20.00</label>
						</div>
						<div className="flex gap-2 items-center">
							<label htmlFor="mx" className="">
								Max. $
							</label>
							<input type="text" className="h-6 w-28 text-lg text-HLig px-2 font-semibold outline-none rounded-md" placeholder="Max Value" id="mx" />
						</div>
						<div className="flex gap-2 items-center">
							<label htmlFor="mn" className="">
								Min. $
							</label>
							<input type="text" className="h-6 w-28 text-lg text-HLig px-2 font-semibold outline-none rounded-md" placeholder="Min Value" id="mn" />
						</div>
						<div className="flex items-center gap-2">
							<input type="checkbox" name="" id="star" className="accent-navigator w-4 h-4 rounded-full" />
							<label htmlFor="star" className="flex items-center">
								<ion-icon name="star" className="text-yellow-300 mr-1"></ion-icon>4 ke atas
							</label>
						</div>
						<div className="w-full flex flex-col gap-2 mt-2">
							<button className="w-full h-10 font-semibold rounded-lg text-white border-[2px] border-navigator hover:bg-transparent hover:text-navigator transition-all duration-300 bg-navigator">Generate filter</button>
							<button className="w-full h-10 font-semibold rounded-lg border-[2px] border-noHover text-noHover dark:border-HDark dark:text-HDark hover:bg-transparent hover:text-navigator hover:border-navigator transition-all duration-300 ">
								Reset filter
							</button>
						</div>
					</div>
				...
				className="w-full md:w-[calc(100%-320px)]  ">
