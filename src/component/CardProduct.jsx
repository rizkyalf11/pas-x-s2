...
const CardProduct = ({product}) => {
  ...
  return (
    <div className="bg-white select-none dark:bg-darkNav shadow-xl h-[250px] rounded-2xl relative flex transition-all duration-300 overflow-hidden group/parent overflow-y-hidden">
      {product.isFav? (
      ...
      <div className=" flex items-center pl-1 gap-1">
        <svg className=' w-[15px] hpsk:w-[19px]' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <h2 className=" font-quicksand font-semibold text-base hpsk:text-xl dark:text-white">{product.rate}</h2>
        <p className="font-quicksand text-TexLig dark:text-TexDark opacity-70 text-[10px] hpsk:text-sm ml-1">{product.buyer}+ users</p>
      </div>
        {product.stok == 0 ? (
          ...
          <div onClick={() => hdlAddToCart(product)} className="flex select-none cursor-pointer justify-between w-full">
            <div className="w-full h-[38px] rounded-xl bg-navigator flex justify-center items-center gap-2 px-[5px] hover:bg-transparent border-[2.5px] border-navigator hover:border-navigator transition-all duration-300 group">
              ...
              <h1 className="text-white cursor-pointer text-[13px] min-[410px]:text-base font-medium  group-hover:text-navigator transition-colors duration-300">Add to Cart</h1>
            </div>
          </div>
        )}
      </div>
