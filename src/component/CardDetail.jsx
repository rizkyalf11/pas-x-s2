...
const CardDetail = ({ product }) => {
	...
	return (
		...
		className="fixed bg-black inset-0 bg-opacity-70 z-50">
			{/* <ion-icon name="close-outline" className="absolute top-4 right-4 text-white text-4xl md:text-5xl bg-black bg-opacity-60 rounded-full z-50"></ion-icon> */}
			<svg className='fill-white absolute top-8 right-8 w-[30px] md:w-[50px] cursor-pointer z-50 bg-black/50 p-1 rounded-full' onClick={() => detailCard && disptach(changeIsShowCard())} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			{/* <div className="absolute top-4 right-4 text-white text-4xl md:text-5xl cursor-pointer bg-opacity-60 rounded-full z-50" onClick={() => disptach(changeIsShowCard())}>❌</div> */}
			<motion.div
			...
			className="absolute flex inset-y-2 inset-x-2 md:inset-y-24 lg:inset-x-20 2xl:inset-x-44 bg-white dark:bg-darkNav rounded-2xl flex-col md:flex-row ">
				<div className="w-full h-2/6 md:w-2/5 md:h-full aspect-square rounded-2xl overflow-hidden relative">
					{/* <img src={dataProd.image} alt="" className="w-full h-full object-cover object-center" /> */}
					...
					<div className=" h-[75%] mt-4 overflow-y-auto scrollbar-thin scrollbar-thumb-navigator">
						<div className=" w-full h-auto box-border">
							...
							<div className=" flex items-center gap-2 my-1">
								{/* <ion-icon name="star" className="text-yellow-300 text-2xl"></ion-icon> */}
					...
					<div className="w-full flex justify-between gap-3 items-center">
						<div className="flex gap-3 flex-row">
							<button className="h-[50px] w-[60px] sm:w-[170px] border-[2.5px] border-noHover dark:border-white rounded-xl flex justify-center items-center gap-2 dark:hover:border-navigator hover:border-navigator group transition-colors duration-300 ">
								{/* <ion-icon name="heart" className=" font-black text-noHover dark:text-white text-3xl hp:text-3xl group-hover:text-navigator transition-colors duration-300"></ion-icon> */}
							...
							<button className="h-[50px] w-[180px] border-[2.5px] border-navigator bg-navigator hover:bg-transparent rounded-xl flex justify-center items-center gap-2 dark:hover:border-navigator hover:border-navigator group transition-colors duration-300 ">
								{/* <ion-icon name="cart" className=" font-black text-white text-3xl hp:text-3xl group-hover:text-navigator transition-colors duration-300"></ion-icon> */}
						...
						<ion-icon name="chatbubble" className=" font-black text-noHover dark:text-white text-4xl hp:text-4xl hover:text-navigator transition-colors duration-300 "></ion-icon>
					</div>
				</div>
			</motion.div>
		</motion.div>
